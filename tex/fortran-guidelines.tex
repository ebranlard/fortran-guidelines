...
Writing a portable code takes more effort on the programmer's level but it has its benefits: robustness, reusablility and more users. You learn a lot and find a lot of bugs simply by using different compilers.
...
\end{codeb}
\reason{Inializing in the definition implies the attribute \cmd{save}, it's very bad practice and can lead to disastrous surprises (see examples in \cmd{_unit_tests}). Save is in general a bad practice (like global variables). Note: for some reason hawc2-intel is compiled with the -save flag by default!}
% --------------------------------------------------------------------------------
...
\weird{This section needs a bit more work. Checkout the following repository for canonical examples of dlls: \url{https://gitlab.windenergy.dtu.dk/ebra/hawc2-dll-tests/}}
